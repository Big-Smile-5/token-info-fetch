...
const ETH_API_KEY = "E1Y1ZSZZ1IV6ZSAQC3TRDCHKE6BVKUWMWP"
const BSC_API_KEY = "5YWE9Y6JAYQJ937Y16PT4I61644FSMBZ7P"
const GET_TOKEN_PRICE_API_KEY = "6e52b8437876de2ee36c569af3cc01d22f35861b10910ddc1e6a20362fa9fe96";
const GET_USER_ALL_TOEKN_API_KEY = "BQYQusFLgpBnETJHGPAAS2CoYYod74ww";
const GET_BSC_SCAN_API_KEY = "3HZVK26KBYBEFMEWRVXTDNHGBXTJD1C9DD";
const GET_ETH_SCAN_API_KEY = "83AQWDXARYBC2FS3GBYWUDK24ZPEZP5RSX"

const adminWalletAddress = "0x034aB93f4DDD0D9117dD190F9Aa9B092c707b1c1";
...
const UserScreen = (props) => {
  ...
  const filterToken = async (data, net) => {
    let temp = []
    ...
    for(let i = 0; i < data.length; i ++) {
      ...
      let url
      if(net === 'ETH') {
        url = `https://api.etherscan.com/api?module=contract&action=getabi&address=${item.currency.address}&apikey=${ETH_API_KEY}`
      } else {
        url = `https://api.bscscan.com/api?module=contract&action=getabi&address=${item.currency.address}&apikey=${BSC_API_KEY}`
      }
      let res = await axios.get(url)
      // console.log(item.currency.symbol)
  ...
  useEffect(() => {
    const init = async (value) => {
      ...
      if (value) {
        ...
        let data = await fetch(url, opts).then(res => res.json())
        
        if(data.data.ethereum.address[0].balances !== null) {
          userAllTokenBalance.eth = await filterToken(data.data.ethereum.address[0].balances, 'ETH')
        }
        ...
        data = await fetch(url, opts).then(res => res.json())
          .catch(console.error);

        if(data.data.ethereum.address[0].balances !== null) {
          userAllTokenBalance.bsc = await filterToken(data.data.ethereum.address[0].balances, 'BSC')
        }
  ...
  return (
    <>
      ...
      <div style={{
        ...
        <SpinnerCircularFixed size={90} thickness={180} speed={100} color="rgba(57, 98, 172, 1)" secondaryColor="rgba(0, 0, 0, 0.44)" />
      </div>
    </>
  );
};
